...
function CommentField({ contentID, parentCommentId, setShowComment }: CommentProps) {
  ...
  const saveNewChildComment = () => {
    ...
    if (newComment != '') {
      ...
      content_store.addNewComment(contentID, finalComment, parentCommentId)
      // const finalComment = result.replace(new RegExp(`${v}\\S*`, 'gi'),'[$&](/games/$&)')
        ...
        {commentText !== "" && commentText.replace(/\s+/g, ' ').trim() !== "" ?
          <div className={styles.func_button_wrapper}>
            <FunctionalGameButton type={'button'} bg_color={'#D6D6D6'} fontSize={12}
              onClick={() => (saveNewChildComment(), setShowComment(false), setCommentText(''))}
            >
              Отправить
